...
public class EmployeeController {
    ...
    public List<EmployeeModel> demo(@PathVariable String name){
        return employeeService.projectEmployeeRelation(name);
    }
    @GetMapping("search-employee/{name}")
    public EmployeeModel searchEmployeeByName(@PathVariable String name){
        return employeeService.searchEmployeeByName(name).orElseThrow();
    }
    @GetMapping("/*")
